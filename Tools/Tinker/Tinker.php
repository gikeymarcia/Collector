...
<p class="namingIsYourBusiness">This is a paragrpah. In the code note how this
  ...
  look above at the CSS you'll see how we can reference a class by putting a
  <code>.</code> before the name (e.g., <code>.namingIsYourBusiness { }</code>)
</p>

<p class="namingIsYourBusiness">Cascading Style Sheets (CSS) are used to control
  the look of HTML elements on a page. By keeping the strcutre (HTML) and
  appearance (CSS) of a page separate it makes it easier to change the look of a
  page without having to completely rewrite the HTML. CSS is usually written so
  that each line sets a different property following the style of
  <code>property: value;</code>.<br>That was a line break. It allows the
  following text to be on a new line but without the space associated with
  a new paragraph (i.e. <code>margin: 15px;</code>).<br> Follow the steps below
  to edit your first CSS code.
</p>
...
<?php
/*
 * Use this area to test out your php code ideas. All of this code is
 * executed AFTER the code above. If you want to run php before, or even
 * during, the rendering of the HTML code, you need to put your code between PHP
 * tags <?php CODE HERE ?> where you want it.
 */

// this is the most basic way you can tell PHP to output some HTML
...
// we can also use IF, ELSEIF, and ELSE statements to give different HTML
// depending on variable values
if ($name == "Mikey") {
