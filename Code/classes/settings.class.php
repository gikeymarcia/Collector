...
class settings
{
    ...
    {
        if ($input == null) { $input = $this->defaultPass; }
    ...
    {
        $var = trim(strtolower($var));
        $location = (isset($this->defaultCommon[$var])) ? "common" : "experiment";
        switch ($location) {
            case 'common':
                $source =& $this->common;
                break;
        ...
        if (is_bool($current)) {                // only allow bools to replace bools
            if (is_bool($val)) {
                ...
                $msg = "Your setting, $var, should be a boolean but it is a $type.";
                trigger_error($msg, E_WARNING);
            }
            ...
            if (is_numeric($val)) {
                ...
                $msg = "Your setting, $var, should be a number but it is a $type.";
                trigger_error($msg, E_WARNING);
            }
        } else {
            $source['data'][$var] = $val;       // allow all other types to be juggled
        }
    }
