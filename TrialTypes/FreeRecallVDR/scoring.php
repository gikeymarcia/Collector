...
function DamLevLimit($str1, $str2, $limit)
{
    ...
    if (abs(strlen($str1) - strlen($str2)) > $limit) {
        return false;
    }
    if ($str1 === '') {
...
foreach ($answers as $i => $ans) {
    foreach ($respExp as $j => $res) {
        if (in_array($res, $unacceptable[$i])) {
            continue;
        }
        if ((substr($res, -1) === 'y' and substr($ans, -3) === 'ies') or (substr($ans, -1) === 'y' and substr($res, -3) === 'ies')) {
            $plural = 2;
        } else {
            $plural = 0;
        }     //no penalties for knowing how to spell
